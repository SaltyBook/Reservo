...
namespace Reservo.Services.PathService
{
    public class PathService : IPathService
    {
        ...
        public string GetReservationPath(Entry entry)
        {
            return Path.Combine(Paths.ManagementPath, $"{entry.Year}-Reservierung", entry.Id.ToString("D2") + "-Reservierung-" + entry.GuestInfo.LastName + ".docx");
        }
        ...
        public string GetInvoicePath(Entry entry)
        {
            int index = -1;
            if (entry.BillingInfo.InvoiceNumber is not null && entry.BillingInfo.InvoiceNumber != 0)
            {
                index = (int)entry.BillingInfo.InvoiceNumber;
            }
            else
            {
                index = GetInvoiceCount(entry.Year);
            }
            return Path.Combine(Paths.ManagementPath, $"{entry.Year}-Rechnung", index.ToString("D2") + "-Rechnung-" + entry.GuestInfo.LastName + ".docx");
        }
        ...
        public int GetInvoiceCount(short year)
        {
